...
router.post('/blackout', authenticate, (req, res) => {
  try {
    ...
    const targets = getBroadcastTargets(studioId);
    const capture = db.prepare('UPDATE screens SET pre_blackout_layout_id = current_layout_id WHERE id = ? AND (pre_blackout_layout_id IS NULL)');
    for (const scr of targets) capture.run(scr.id);   // only captures if not already blacked

    if (!synthetic && targets.length) {
      const ph = targets.map(() => '?').join(',');
      db.prepare(`UPDATE screens SET current_layout_id = ?, updated_at = datetime('now') WHERE id IN (${ph})`).run(layout.id, ...targets.map(s => s.id));
    }

    const modules = Array.isArray(layout.modules) ? layout.modules : JSON.parse(layout.modules || '[]');
    const enriched = enrichLayout({ ...layout, modules }, null);
    for (const scr of targets) {
      getIO().to(`screen:${scr.id}`).emit('set_layout', { layoutId: layout.id, layout: enriched, source: 'blackout' });
      getIO().to(`studio:${studioId}`).emit('screen_preview', { screenId: scr.id, layoutId: layout.id, layout: enriched, timestamp: new Date().toISOString() });
    }
...
// POST /restore — put every captured screen back to EXACTLY the layout it was showing
// before the blackout. Not "the first non-blackout layout" — the one that was actually
// up, per screen. Clears the capture markers so a later blackout captures fresh.
router.post('/restore', authenticate, (req, res) => {
  try {
    ...
    const captured = db.prepare('SELECT id, pre_blackout_layout_id FROM screens WHERE studio_id = ? AND pre_blackout_layout_id IS NOT NULL').all(studioId);
    if (!captured.length) return res.json({ message: 'Nothing to restore — no blackout was captured', restored: 0 });

    let restored = 0;
    for (const scr of captured) {
      const layout = getLayoutById(scr.pre_blackout_layout_id);
      if (layout) {
        const enriched = enrichLayout({ ...layout, modules: JSON.parse(layout.modules) }, scr.id);
        db.prepare("UPDATE screens SET current_layout_id = ?, pre_blackout_layout_id = NULL, updated_at = datetime('now') WHERE id = ?").run(layout.id, scr.id);
        getIO().to(`screen:${scr.id}`).emit('set_layout', { layoutId: layout.id, layout: enriched, source: 'restore' });
        getIO().to(`studio:${studioId}`).emit('screen_preview', { screenId: scr.id, layoutId: layout.id, layout: enriched, timestamp: new Date().toISOString() });
        restored++;
      } else {
        // The captured layout was deleted while black. Don't strand the screen on black
        // forever — clear the marker so the operator can push something new.
        db.prepare('UPDATE screens SET pre_blackout_layout_id = NULL WHERE id = ?').run(scr.id);
      }
    }
...
router.get('/blackout/state', authenticate, (req, res) => {
  ...
  const blacked = db.prepare('SELECT COUNT(*) n FROM screens WHERE studio_id = ? AND pre_blackout_layout_id IS NOT NULL').get(studioId).n;
