  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', fontFamily: HEAD, color: '#fff',
      ...
      <style>{`
        ...
        @keyframes ggShine { from { transform: translateX(-130%) skewX(-20deg); } to { transform: translateX(320%) skewX(-20deg); } }
      ...
      <div style={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1vh' }}>
        {/* GOAL! */}
        <div style={{ position: 'relative', animation: 'ggGoal 0.7s cubic-bezier(.2,1.4,.4,1) both', overflow: 'hidden' }}>
          ...
          <span style={{ position: 'absolute', top: 0, left: 0, width: '20%', height: '100%', background: 'rgba(255,255,255,0.55)', animation: 'ggShine 1s ease-in 0.5s both' }} />
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2vw', animation: 'ggUp 0.6s ease-out 0.25s both' }}>
          ...
          <div style={{ textAlign: 'left' }}>
            ...
            <div style={{ fontWeight: 700, fontSize: '9vh', lineHeight: 0.95, textShadow: '0 4px 20px rgba(0,0,0,.5)' }}>
              {player.number ? <span style={{ color: GOLD }}>{player.number} </span> : null}{player.name}
            </div>
