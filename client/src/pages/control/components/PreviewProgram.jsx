...
export default function PreviewProgram({ previewLayout, programLayout, onTake, onCut, transitionType, transitionDuration }) {
  return (
    <div className="grid grid-cols-2 gap-4 mb-6">
      ...
      <div className="bg-gray-900 rounded-xl border border-yellow-800/50 overflow-hidden">
        ...
        <div className="aspect-video bg-black flex items-center justify-center relative">
          {previewLayout ? (
            <ScreenPreview layout={previewLayout} />
          ) : (
            <p className="text-gray-600 text-sm">Click a layout to preview</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-gray-900 rounded-xl border border-red-800/50 overflow-hidden">
        ...
        <div className="aspect-video bg-black flex items-center justify-center">
          {programLayout ? (
            <ScreenPreview layout={programLayout} />
          ) : (
            <p className="text-gray-600 text-sm">No layout live</p>
          )}
        </div>
      </div>
      ...
      <div className="col-span-2 flex items-center justify-center gap-3">
        <button
          ...
          className="px-8 py-2 bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:text-gray-600 text-white text-sm font-bold rounded-lg transition-colors uppercase tracking-wider"
        >
          Cut
        </button>
        <button
          onClick={onTake}
          disabled={!previewLayout}
          className="px-12 py-3 bg-red-600 hover:bg-red-500 disabled:bg-red-900 disabled:text-red-700 text-white font-bold rounded-lg transition-all uppercase tracking-wider text-lg shadow-lg shadow-red-600/30"
        >
          TAKE
        </button>
        <span className="text-gray-500 text-xs ml-2">
          {transitionType} · {transitionDuration}s
        </span>
      </div>
    </div>
  );
