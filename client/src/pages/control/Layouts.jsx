    ...
    if (selectedId) {
      api
        ...
        .then((data) => {
          ...
          setScreenOrientation(layout.orientation === 'portrait' ? 'portrait' : 'landscape');
  ...
  async function handleDuplicateLayout(id) {
    ...
    try {
      ...
      const data = await api.post('/layouts', {
        ...
        resolution_w: src.resolution_w || 1920,
        resolution_h: src.resolution_h || 1080,
        public_safe: src.public_safe || 0,
        project: src.project || '',
      });
      fetchLayouts();
      setSelectedId(data.layout?.id || data.id);
      ...
      <div className="w-60 border-r border-gray-800 flex flex-col shrink-0"
        ...
        <div className="flex-1 overflow-y-auto hide-scrollbar px-2 py-2 space-y-1">
          ...
          {(() => {
            ...
            const visible = q
              ? layouts.filter((l) => (l.name || '').toLowerCase().includes(q) || projectOf(l).toLowerCase().includes(q))
              : layouts;
            const groups = {};
            for (const l of visible) { const k = projectOf(l); (groups[k] = groups[k] || []).push(l); }
            ...
            if (q && order.length === 0) {
              return (
                <div className="text-center py-8 px-3">
                  <p className="text-gray-500 text-xs">No layouts match “{searchQuery}”</p>
                </div>
              );
            }
            ...
            return order.map((proj) => {
              ...
              const collapsed = !q && collapsedProjects.has(proj);
              return (
                ...
                <div className="flex items-center gap-1 bg-gray-800/50 rounded-lg px-1 py-0.5 border border-gray-700/50">
                  <button
                    onClick={() => {
                      ...
                      updateLayout({ orientation: 'landscape', resolution_w: 1920, resolution_h: 1080 });
                    }}
                  ...
                  <button
                    onClick={() => {
                      ...
                      updateLayout({ orientation: 'portrait', resolution_w: 1080, resolution_h: 1920 });
                    }}
