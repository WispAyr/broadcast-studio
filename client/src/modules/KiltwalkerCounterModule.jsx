...
const MILESTONES = [100, 250, 500, 750, 1000, 1250, 1500, 1750, 2000, 2250, 2500, 2750, 3000, 3250, 3500, 3750, 4000, 4250, 4500, 4750, 5000, 5250, 5500, 5750, 6000, 6250, 6500, 6750, 7000, 7250, 7500, 7750, 8000, 8500, 9000, 9500, 10000];
...
export default function KiltwalkerCounterModule({ config = {} }) {
  ...
  useEffect(() => {
    ...
    if (mode === 'exact') {
      ...
      // For milestone mode: ramp displayCount toward target. Below the first
      // milestone (MILESTONES[0]) we step every render so the raw number reads
      // correctly during pre-event / low-volume moments. Above it we apply the
      // smoothBuffer so we don't visually flicker the number once a milestone
      // takes over the display.
      const belowFirstMilestone = target < MILESTONES[0];
      if (target !== displayCount && (belowFirstMilestone || target - displayCount >= smoothBuffer || displayCount === 0)) {
        const step = Math.max(1, Math.ceil(Math.abs(target - displayCount) / 30));
        const timer = setTimeout(() => {
          setDisplayCount(prev => Math.min(prev + step, target));
        }, 20);
